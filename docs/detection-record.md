...
| In-place display updates duplicated on the upgrade log — a 7-line Rich Live block landed 77 lines with 0 repaints: live renderers rewind AFTER drawing (`bot<CR><ESC>[2K<ESC>[1A…`), but movement was consumed only at the buffer head, so trailing choreography stayed as text and `back` was always 0. Same class second half: stderr bypassed the painter (per-chunk `AuxLine`), so `\r` progress there appended per tick | user (`update-local` double printouts) | movement consumed at every `\r`-state head + open-line addressing (`painted.rs`); one shared `paint_msg` routing; stderr through the same painter with styling kept (`exec_runner`); `repaint_test` Rich-shape e2e plus stderr unit tests |
| The `unhealthy` filter test failed whenever the box was loaded -- and correctly so: `unhealthy` is a synthetic token matching breaching hosts, and parallel suites push MEM past 85%. The test asserted the healthy outcome unconditionally, reporting a working filter as broken | gate red during a release (MEM 88% on screen) | assert the correlation instead of one branch: filter badge present, and panel visible XOR "No host matches" |
| Lingering agent processes and upgrade view stuck in STARTED: background processes (Docker, subshells) held PTY/stderr descriptors open after child shell exited; agent `pump()` hung waiting for EOF | user | `pump()` actively polls `try_wait(child.pid)`, drains remaining bytes with zero timeout, closes descriptors, and exits cleanly |
| Double lines on `update-local` and live-rendered tables: `run_upgrade` passed `cols: 0, rows: 0` which clamped to 1x1 in PTY, wrapping every character into a new line and breaking ANSI cursor rewinds | user | PTY defaults 0 dimensions to 80x24; `run_upgrade` and `exec_runner` set 80x24 explicitly |
