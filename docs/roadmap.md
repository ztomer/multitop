...
## 1. Rotate the vault master password

**Status:** implemented in the library, unreachable from the app.

`Vault::change_password` exists in `crates/vault/src/api.rs` and is tested, but
no UI path calls it. The vault holds every stored sudo password and its master
password can never be changed, so a master password that has been exposed
cannot be retired without deleting the vault and re-entering every credential.

**What is missing:** a key in the Configuration panel that prompts for the
current password, then the new one, and calls `change_password`. It must fail
closed — if the re-wrap or save fails, the old password has to keep working.
Check what happens to the Secure Enclave wrapper on rotation: the vault key
changes, so the enclave wrapper needs re-binding in the same operation or
biometric unlock breaks.

Previously tracked as "[R] Rotate Key button in Settings UI" in the vault
roadmap.

## 2. Import hosts from `~/.ssh/config`
...
## 4. Decide the fate of two unused vault API functions
...
worked down by hand. Items 1–4 above are its interesting entries; the rest are
