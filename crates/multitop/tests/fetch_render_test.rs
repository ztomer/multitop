...
fn os_macos_gets_apple_logo() {
    ...
    assert!(
        first.contains(&'c'),
        "macOS logo should show Apple: {first:?}"
    );
}
...
fn os_darwin_kernel_gets_apple_logo() {
    ...
    assert!(
        first.contains(&'c'),
        "Darwin kernel should map to Apple logo: {first:?}"
    );
}
...
fn os_ubuntu_derivatives_get_own_logos() {
    ...
    assert_ne!(
        kubuntu_line, ubuntu_line,
        "Kubuntu and Ubuntu should have different logos"
    );
}
...
fn os_windows11_gets_windows_logo() {
    ...
    let os_line = plain(&out)
        ...
        .unwrap_or_default();
    assert!(
        os_line.contains("Windows"),
        "Windows 11 OS name should appear"
    );
...
fn os_raspbian_gets_debian_logo() {
    ...
    let os_line = plain(&out)
        ...
        .unwrap_or_default();
    assert!(
        os_line.contains("Raspbian"),
        "Raspbian OS name should appear"
    );
    let p = logo_line(&out, 1);
    assert!(
        p.contains("ooo") || p.contains("::"),
...
fn os_unknown_os_still_renders() {
    ...
    assert!(
        header.contains(&fullwidth("user@host")),
        "header present for unknown OS"
    );
}
...
fn alignment_header_is_centered() {
    ...
    assert!(
        header.contains(&fw),
        "header should have fullwidth hostname"
    );
}
...
fn sizing_logo_lines_match_detail_lines_when_logo_is_tall() {
    ...
    assert!(
        !logo_first.trim().is_empty() || !logo_last.trim().is_empty(),
