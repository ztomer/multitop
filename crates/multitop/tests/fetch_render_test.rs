...
#[test]
fn color_bar_appears_when_enough_room() {
    let out = fetch_render::render_fetch(&snap("Ubuntu 24.04", "6.8.0"), 80, 24, &ANSI);
    let last = out.last().expect("at least one line");
    assert!(last.contains("\x1b[40m"), "color bar should have black block");
    assert!(last.contains("\x1b[47m"), "color bar should have white block");
}

#[test]
fn color_bar_omitted_when_no_room() {
    let out = fetch_render::render_fetch(&snap("Ubuntu 24.04", "6.8.0"), 80, 8, &ANSI);
    let last = out.last().expect("at least one line");
    assert!(!last.contains("\x1b[40m"),
        "color bar should be omitted at 80x8: {last}");
}

// ------------------------------------------------------------------------
//  PLAIN PALETTE — no ANSI escapes (except the color bar which is intentional)
// ------------------------------------------------------------------------

#[test]
fn plain_palette_no_escapes() {
    let out = fetch_render::render_fetch(&snap("Ubuntu 24.04", "6.8.0"), 80, 24, &PLAIN);
...
#[test]
fn plain_palette_color_bar_still_has_escapes() {
    let out = fetch_render::render_fetch(&snap("Ubuntu 24.04", "6.8.0"), 80, 24, &PLAIN);
    let last = out.last().expect("at least one line");
    assert!(last.contains("\x1b[40m"), "color bar always has ANSI colors");
}
...
#[test]
fn edge_empty_kernel_does_not_panic() {
    let out = fetch_render::render_fetch(&snap("Ubuntu 24.04", ""), 80, 24, &ANSI);
    assert!(out.len() >= 2, "empty kernel should render: {out:?}");
}

#[test]
fn edge_empty_os_does_not_panic() {
    let out = fetch_render::render_fetch(&snap("", "6.8.0"), 80, 24, &ANSI);
    assert!(out.len() >= 2, "empty OS should render: {out:?}");
}

#[test]
fn edge_very_long_os_name_truncates_gracefully() {
    let long_os = "SuperDuperExtraLongOperatingSystemName Ultimate Edition 99.99 LTS".to_string();
    let out = fetch_render::render_fetch(
        &snap(&long_os, "6.8.0"), 80, 24, &ANSI
    );
    assert!(out.len() >= 2, "long OS name should render: {out:?}");
    let os_line = plain(&out).iter().find(|l| l.contains("OS")).cloned().unwrap_or_default();
    assert!(os_line.contains("Super"), "OS name should appear in output");
}

#[test]
fn edge_narrow_40cols_does_not_truncate_critically() {
    let out = fetch_render::render_fetch(&snap("Ubuntu 24.04", "6.8.0"), 40, 24, &ANSI);
    assert!(out.len() >= 3, "narrow should have header + some details");
    // All detail rows present (may be wider than 40 cols but that's fine)
    for label in &["OS", "Kernel", "Uptime", "Host", "CPU", "Memory", "Disk"] {
        let found = plain(&out).iter().any(|l| l.contains(label));
        assert!(found, "narrow should show {label}");
    }
}
