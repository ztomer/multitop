...
/// The reader's state across one pump: the two sieves, the read buffer,
/// what stdout said before the shell finished starting, and the flags the
/// markers set. One value, so the per-chunk steps are methods instead of a
/// ten-argument hand-off.
struct Reader<'a> {
    req: &'a Request<'a>,
    sieve: Sieve,
...
pub fn pump<W: Write>(
    ...
    while child.master >= 0 || child.errpipe >= 0 {
        ...
        if m_ready {
            reader.read_master_chunk(child, out, seq);
        }

        if e_ready {
            reader.read_errpipe_chunk(child, out, seq);
        }
        ...
        if reaped.is_some() {
            reader.drain_post_exit(child, out, seq);
            break;
        }
    ...
    (reaped, reader.sudo_rejected)
}

impl Reader<'_> {
    fn drain_post_exit<W: Write>(&mut self, child: &mut pty::Child, out: &mut W, seq: &mut u32) {
        while child.master >= 0 || child.errpipe >= 0 {
            ...
            if m_drain {
                self.read_master_chunk(child, out, seq);
            }
            if e_drain {
                self.read_errpipe_chunk(child, out, seq);
            }
        }
        if child.master >= 0 {
            self.close_master(child, out, seq);
        }
        if child.errpipe >= 0 {
            let tail = self.err_sieve.finish();
            emit_stderr(&tail, out, seq, &mut self.sudo_rejected);
            unsafe { libc::close(child.errpipe) };
            child.errpipe = -1;
        }
    }

    /// Flush the stdout sieve, forward what was held, and close the master.
    fn close_master<W: Write>(&mut self, child: &mut pty::Child, out: &mut W, seq: &mut u32) {
        let tail = self.sieve.finish();
        self.consume(&tail, out, seq, child.master);
        release(&mut self.held, out, seq);
        unsafe { libc::close(child.master) };
        child.master = -1;
    }

    fn read_master_chunk<W: Write>(&mut self, child: &mut pty::Child, out: &mut W, seq: &mut u32) {
        match pty::read_fd(child.master, &mut self.buf) {
            Ok(0) | Err(_) => self.close_master(child, out, seq),
            Ok(n) => {
                let sifted = self.sieve.feed(&self.buf[..n]);
                self.consume(&sifted, out, seq, child.master);
            }
        }
    }

    fn read_errpipe_chunk<W: Write>(&mut self, child: &mut pty::Child, out: &mut W, seq: &mut u32) {
        match pty::read_fd(child.errpipe, &mut self.buf) {
            Ok(0) | Err(_) => {
                let tail = self.err_sieve.finish();
                emit_stderr(&tail, out, seq, &mut self.sudo_rejected);
                unsafe { libc::close(child.errpipe) };
            ...
            Ok(n) => {
                let sifted = self.err_sieve.feed(&self.buf[..n]);
                emit_stderr(&sifted, out, seq, &mut self.sudo_rejected);
            }
        }
    }
    ...
    fn consume<W: Write>(&mut self, pieces: &[Piece], out: &mut W, seq: &mut u32, master: RawFd) {
        for piece in pieces {
            match piece {
                Piece::Out(bytes) => {
                    if !self.done {
                        stash(&mut self.held, out, seq, bytes);
                    }
                }
                ...
                Piece::Mark(MarkerKind::Started) => self.held = None,
                Piece::Mark(MarkerKind::Done) => self.done = true,
                Piece::Mark(k) => {
                    send(out, &ExecFrame::Marker(*k));
                    match k {
                        MarkerKind::PwReady if !self.password_sent => {
                            // Echo is off on the far side now; before this point
                            // the pty would print the password straight back into
                            // the operator's log.
                            if let Some(p) = self.req.password {
                        ...
                        MarkerKind::SudoFailed => self.sudo_rejected = true,
                        _ => {}
                    }
                }
            }
